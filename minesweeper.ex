defmodule Minesweeper do
  ...
  def valid_moves(tam,l,c), do:
    Enum.filter((for movL <- [-1, 0, 1], movC <- [-1, 0, 1], {movL, movC} != {0,0}, do: {l + movL, c + movC}), fn {x, y} -> is_valid_pos(tam, x, y) end)

  # a comprehension serve para montar a matriz com todas as 8 posições possíveis em volta do pontos selecionado (excluindo o próprio ponto)
  # a proxima parte do filter serve pra devolver apenas as tuplas com posições válidas de acordo com a função is_valid_pos
  ...
  def abre_jogada(l,c,minas,tab) do
    cond do
      ...
      True -> abre_jogada(l, c, minas, abre_posicao(tab, minas, l, c))
    end
  end
...
# - Se a posição {l,c} está fechada (contém "-"), escrever o número de minas adjascentes a esssa posição no tabuleiro (usar conta_minas)
  ...
  def abre_posicao(tab,minas,l,c) do
    cond do
      get_pos(minas, l, c) == true -> update_pos(tab, l, c, "*")
...
# contém "-"). Usar gera_lista
  ...
  def gera_tabuleiro(n), do: gera_lista(n-1, gera_lista(n-1, "-"))

# -- gera_mapa_de_minas/1: recebe o tamanho do tabuleiro e gera um mapa de minas zero, onde todas as posições contém false

  def gera_mapa_de_minas(n), do: gera_lista(n-1, gera_lista(n-1, false))
...
# conta_fechadas/1: recebe um tabueleiro de jogo e conta quantas posições fechadas existem no tabuleiro (posições com "-")
  ...
  def conta_fechadas(tab) do
    Enum.reduce(tab, 0, fn line, acc ->
      acc + Enum.reduce(line, 0, fn pos, acc_inner ->
        cond do
          pos == "-" -> acc_inner + 1
          True -> acc_inner
        end
...
# -- conta_minas/1: Recebe o tabuleiro de Minas (MBoard) e conta quantas minas existem no jogo
  ...
  def conta_minas(minas) do
    Enum.reduce(minas, 0, fn line, acc ->
      acc + Enum.reduce(line, 0, fn pos, acc_inner ->
        cond do
          pos == true -> acc_inner + 1
          True -> acc_inner
        end
      end)
    end)
